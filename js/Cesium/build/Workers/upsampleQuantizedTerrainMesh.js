define([
  ...
  "./OrientedBoundingBox-c2ccbf3c",
  "./createTaskProcessorWorker",
  "./combine-6d9c3903",
  "./WebGLConstants-f63312fc",
  "./EllipsoidTangentPlane-7d47054e",
  "./AxisAlignedBoundingBox-0eaa4f31",
  "./IntersectionTests-be7f8dfa",
  "./Plane-a66da2a9",
], function (e, t, n, i, s, r, h, o, u, p, d, l, a, c, f, g) {
  "use strict";
  const m = {
      ...
      computeBarycentricCoordinates: function (e, t, s, r, h, o, u, p, d) {
        const l = s - u,
          a = u - h,
          c = o - p,
          f = r - p,
      ...
      computeLineSegmentLineSegmentIntersection: function (
        ...
        const l = (p - o) * (s - e) - (u - h) * (r - t);
        if (0 === l) return;
        const a = ((u - h) * (t - o) - (p - o) * (e - h)) / l,
          c = ((s - e) * (t - o) - (r - t) * (e - h)) / l;
        return a >= 0 && a <= 1 && c >= 0 && c <= 1
          ? (i.defined(d) || (d = new n.Cartesian2()),
            (d.x = e + a * (s - e)),
            (d.y = t + a * (r - t)),
            d)
          : void 0;
    ...
    B = [],
    y = [],
    I = new n.Cartographic();
  let A = new n.Cartesian3();
  const b = [],
    v = [],
    T = [],
    z = [],
    M = [],
    N = new n.Cartesian3(),
    V = new t.BoundingSphere(),
    E = new u.OrientedBoundingBox(),
    R = new n.Cartesian2(),
    H = new n.Cartesian3();
  function O() {
  ...
  (O.prototype.clone = function (e) {
    ...
    (O.prototype.initializeIndexed = function (e, t, n, i, s) {
      (this.uBuffer = e),
        (this.vBuffer = t),
        (this.heightBuffer = n),
        (this.normalBuffer = i),
    ...
    (O.prototype.initializeFromClipResult = function (e, t, n) {
      let i = t + 1;
      return (
        -1 !== e[t]
          ? n[e[t]].clone(this)
  ...
  let S = new n.Cartesian2(),
    U = -1;
  const F = [new n.Cartesian3(), new n.Cartesian3()],
    P = [new n.Cartesian3(), new n.Cartesian3()];
  function D(t, i) {
    ++U;
    let s = F[U],
      r = P[U];
    return (
      ...
      (A = n.Cartesian3.lerp(s, r, t.ratio, A)),
      n.Cartesian3.normalize(A, A),
      e.AttributeCompression.octEncode(A, i),
      --U,
      i
    );
  }
  ...
  const W = [];
  function X(e, t, n, s, r, h, o, u, p) {
    if (0 === o.length) return;
    let d = 0,
      l = 0;
    for (; l < o.length; ) l = W[d++].initializeFromClipResult(o, l, u);
    for (let r = 0; r < d; ++r) {
      const o = W[r];
      if (o.isIndexed())
        ...
        if (i.defined(h[r])) o.newIndex = h[r];
          ...
          e.push(o.getU()),
            t.push(o.getV()),
            n.push(o.getH()),
            p && (s.push(o.getNormalX()), s.push(o.getNormalY())),
            (o.newIndex = i),
            (h[r] = i);
        }
      }
  ...
  return (
    ...
    p(function (e, i) {
      const h = e.isEastChild,
        ...
        S = T,
        U = M;
      (f.length = 0), (g.length = 0), (S.length = 0), (U.length = 0);
      const F = z;
      F.length = 0;
      const P = {},
        D = e.vertices;
      ...
      const k = s.TerrainEncoding.clone(e.encoding),
      ...
      let Q, $, ee, te, ne;
      for ($ = 0, ee = 0; $ < Y; ++$, ee += 2) {
        const e = k.decodeTextureCoordinates(D, $, R);
        if (
          ((Q = k.decodeHeight(D, $)),
          (te = o.CesiumMath.clamp((e.x * x) | 0, 0, x)),
          (ne = o.CesiumMath.clamp((e.y * x) | 0, 0, x)),
          (j[$] = o.CesiumMath.clamp((((Q - _) / (G - _)) * x) | 0, 0, x)),
          te < 20 && (te = 0),
          ne < 20 && (ne = 0),
          x - te < 20 && (te = x),
          x - ne < 20 && (ne = x),
          ...
          const e = k.getOctEncodedNormal(D, $, H);
          (q[ee] = e.x), (q[ee + 1] = e.y);
        }
        ((h && te >= w) || (!h && te <= w)) &&
          ((p && ne >= w) || (!p && ne <= w)) &&
      ...
      const ie = [];
      ie.push(new O()), ie.push(new O()), ie.push(new O());
      const se = [];
      let re, he;
      for (
        se.push(new O()), se.push(new O()), se.push(new O()), $ = 0;
        $ < W.length;
        $ += 3
      ) {
        const e = W[$],
          ...
          s = J[t],
          r = J[n];
        ie[0].initializeIndexed(J, Z, j, q, e),
          ie[1].initializeIndexed(J, Z, j, q, t),
          ie[2].initializeIndexed(J, Z, j, q, n);
        const o = m.clipTriangleAtAxisAlignedThreshold(w, h, i, s, r, C);
        (re = 0),
          re >= o.length ||
            ...
            re >= o.length ||
              ...
              re >= o.length ||
                ...
                (he = m.clipTriangleAtAxisAlignedThreshold(
                  w,
                  p,
                  se[0].getV(),
                  se[1].getV(),
      ...
      for (Be < ye && (Be += o.CesiumMath.TWO_PI), $ = 0; $ < f.length; ++$)
        (te = Math.round(f[$])),
          ...
          ne <= a
            ...
            : (ne = 2 * ne + ue),
          (g[$] = ne),
          (Q = o.CesiumMath.lerp(_, G, S[$] / x)),
          Q < ce && (ce = Q),
          Q > fe && (fe = Q),
          (S[$] = Q),
          (I.longitude = o.CesiumMath.lerp(ye, Be, te / x)),
          (I.latitude = o.CesiumMath.lerp(Ce, we, ne / x)),
          (I.height = Q),
          me.cartographicToCartesian(I, A),
          ge.push(A.x),
          ge.push(A.y),
          ge.push(A.z);
      const Ie = t.BoundingSphere.fromVertices(ge, n.Cartesian3.ZERO, 3, V),
        Ae = u.OrientedBoundingBox.fromRectangle(xe, ce, fe, me, E),
        ...
        ve = fe - ce,
        Te = new Uint16Array(f.length + g.length + S.length);
      for ($ = 0; $ < f.length; ++$) Te[$] = f[$];
      let ze = f.length;
      for ($ = 0; $ < g.length; ++$) Te[ze + $] = g[$];
      for (ze += g.length, $ = 0; $ < S.length; ++$)
        Te[ze + $] = (x * (S[$] - ce)) / ve;
      const Me = r.IndexDatatype.createTypedArray(f.length, F);
      let Ne;
      if (K) {
        const e = new Uint8Array(U);
        i.push(Te.buffer, Me.buffer, e.buffer), (Ne = e.buffer);
      } else i.push(Te.buffer, Me.buffer);
      return {
        vertices: Te.buffer,
        encodedNormals: Ne,
        indices: Me.buffer,
